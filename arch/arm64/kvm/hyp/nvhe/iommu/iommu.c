...
static inline int pkvm_to_iommu_prot(enum kvm_pgtable_prot prot, u64 addr)
{
	...
	/* DMA execute permission follows the privileged-execute permission. */
...
{
	/*
	 * Linux 7.1 no longer exposes _iommu_iotlb_add_page(), and the public
	 * helper takes a struct iommu_domain. Keep the Android pKVM semantics
	 * while using the hyp-private domain type.
	 */
